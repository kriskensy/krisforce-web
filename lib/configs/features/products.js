import { Package, Tags, ListTree, Banknote } from "lucide-react";
...
export const PRODUCTS_SUB_FEATURES = {
  'products': {
    ...
    fields: [
      ...
      { name: 'standard_price', label: 'Price', type: 'number', isCurrency: true, required: true },
      { 
        name: 'category_id', 
        label: 'Category', 
        type: 'select', 
        ...
        relationName: 'product_categories',
        displayKey: 'name'
      }
    ]
  },
  ...
  'price_lists_items': {
    ...
    apiEndpoint: '/api/price_lists/[id]/items',
    fields: [
      { 
        name: 'price_list_id', 
        label: 'Price List', 
      ...
      { name: 'price', label: 'Price', type: 'number', isCurrency: true, required: true },
      { name: 'deleted_at', label: 'Deactivated?', type: 'checkbox', defaultValue: false  }
    ]
  },
  'categories': {
