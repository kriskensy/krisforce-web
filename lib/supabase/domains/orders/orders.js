...
export async function getOrders(filters) {
  try {
    ...
    const dataSource = client ? 'report_orders' : 'orders'

    let query = supabase
      .from(dataSource)
      .select(
        client ? '*' : 'id, order_number, order_date, total_amount, status_id, order_statuses (name), client_id, clients (name), orders_client_id_fkey (name), created_by, created_at, deleted_at', { count: 'exact' })
    ...
    if (search) {
      ...
      if (matchedClientIds.length > 0) {
        query = query.or(`order_number.ilike.%${search}%,client_id.in.(${matchedClientIds.join(',')})`);
      } else {
    ...
    //sort + pagination
    const finalOrderBy = (client && orderBy === 'created_at') ? 'order_date' : orderBy;
    query = query
      .order(finalOrderBy, { ascending: order === 'asc' })
...
export async function getOrderById(id) {
  try {
    ...
    const { data, error } = await supabase
      .from('orders')
      .select('id, order_number, order_date, total_amount, status_id, order_statuses (name), client_id, clients (name), orders_client_id_fkey (name), created_by, created_at, deleted_at, order_items (id, quantity, unit_price, total_price, products (name))')
      .eq('id', id)
      .maybeSingle()
