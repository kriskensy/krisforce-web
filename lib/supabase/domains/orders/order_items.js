...
export async function getOrderItems(order_id, filters) {
  try {
    ...
    let query = supabase
      .from('order_items')
      .select('id, order_id, quantity, unit_price, total_price, product_id', { count: 'exact' })
...
export async function updateOrderItem(itemId, orderId, itemData) {
  try {
    ...
    updatePayload.total_price = (updatePayload.quantity || existing.quantity) * 
                              (updatePayload.unit_price || existing.unit_price)
