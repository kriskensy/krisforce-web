...
//TODO testing API: veirfy deactivated. Please reactivate after!
//GET
export async function getUsers(filters = {}) {
  try {
    ...
    let query = supabase
    .from('users')
    .select('id, email, active, created_at, role_id, user_profiles(id, first_name, last_name, phone)', { count: 'exact' })
...
export async function getUserById(id) {
  try {
    ...
    const { data, error } = await supabase
      .from('users')
      .select('id, email, active, created_at, role_id, user_profiles(id, first_name, last_name, phone)')
      .eq('id', id)
      .maybeSingle()
...
export async function createUser(userData) {
  try {
    const { email, password, role_id, active = true, user_profiles = {} } = userData;

    // await verifyAdmin();

    if(!email) throw new Error('Email is require');
    if(!password) throw new Error('Password is require')
    if(password.length < 12) throw new Error('Password must be ay least 8 characters')
