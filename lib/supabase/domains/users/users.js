...
export async function getUsers(filters = {}) {
  try {
    ...
    let query = supabase
    .from('users')
    .select('id, email, active, created_at, role_id, user_profiles(id, first_name, last_name, phone), client_id', { count: 'exact' })
...
export async function getUserById(id) {
  try {
    ...
    const { data, error } = await supabase
      .from('users')
      .select('id, email, active, created_at, role_id, user_profiles(id, first_name, last_name, phone), client_id')
      .eq('id', id)
      .maybeSingle()
