...
export async function getInvoicePayments(invoiceId, filters) {
  try {
    ...
    let query = supabase
      .from('payments')
      .select('id, invoice_id, invoices (invoice_number), client_id, clients (name), amount, payment_date, method_id, payment_methods (code, name), deleted_at', { count: 'exact' })
    ...
    if (search) {
      ...
      const matchedInvoicesIds = matchedInvoices?.map(invoice => invoice.id) || [];

      //get payment methods
      const { data: matchedPaymentMethods } = await supabase
        .from('payment_methods')
        .select('id')
        .ilike('name', `%${search}%`);

      const matchedPaymentMethodsIds = matchedPaymentMethods?.map(paymentMethod => paymentMethod.id) || [];

      const searchResultsArray = [];

      if (matchedInvoicesIds.length > 0)
        searchResultsArray.push(`invoice_id.in.(${matchedInvoicesIds.join(',')})`);

      if (matchedPaymentMethodsIds.length > 0)
        searchResultsArray.push(`method_id.in.(${matchedPaymentMethodsIds.join(',')})`);
...
export async function getInvoicePaymentById(paymentId, invoice_id) {
  try {
    ...
    const { data, error } = await supabase
      .from('payments')
      .select('id, invoice_id, invoices (invoice_number), client_id, clients (name), amount, payment_date, method_id, payment_methods (code, name), deleted_at')
      .eq('id', paymentId)
