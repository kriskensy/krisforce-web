...
export async function getInvoiceItems(invoiceId, filters) {
  try {
    ...
    let query = supabase
      .from('invoice_items')
      .select('id, invoice_id, invoices (invoice_number), description, quantity, unit_price, total_price, product_id, products (name)', { count: 'exact' })
...
export async function getInvoiceItemById(itemId, invoice_id) {
  try {
    ...
    const { data, error } = await supabase
      .from('invoice_items')
      .select('id, invoice_id, invoices (invoice_number), description, quantity, unit_price, total_price, product_id, products (name)')
      .eq('id', itemId)
