...
export async function getInvoiceById(id) {
  try {
    ...
    const { data, error } = await supabase
      .from('invoices')
      .select('id, invoice_number, invoice_date, due_date, total_amount, paid_amount, status_id, invoice_statuses (name), client_id, clients (name), order_id, orders (order_number), created_at, deleted_at')
      .eq('id', id)
      .maybeSingle()
