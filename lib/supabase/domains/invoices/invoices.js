...
export async function getInvoices(filters) {
  try {
    ...
    let query = supabase
      .from(dataSource)
      .select(
        client ? '*' : 'id, invoice_number, invoice_date, due_date, total_amount, paid_amount, status_id, client_id, clients (name), order_id, orders (order_number), created_at, deleted_at', { count: 'exact' })
...
export async function createInvoice(invoiceData) {
  try {
    ...
    //insert invoice items //TODO creta via procedure
