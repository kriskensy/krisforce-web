...
export async function getInvoices(filters) {
  try {
    ...
    let query = supabase
      .from('invoices')
      .select('id, invoice_number, invoice_date, due_date, total_amount, paid_amount, status_id, client_id, clients (name), order_id, orders (order_number), created_at, deleted_at', { count: 'exact' })
      .is('deleted_at', null)

    // invoice number, client name, or order number searching
    if (search) {
      query = query.or(
        `invoice_number.ilike.%${search}%,clients.name.ilike.%${search}%,orders.order_number.ilike.%${search}%`
      )
    }

    //filters
    if (status_code) query.eq('status.code', status_code)
...
export async function getInvoiceById(id) {
  try {
    ...
    const { data, error } = await supabase
      .from('invoices')
      .select('id, invoice_number, invoice_date, due_date, total_amount, paid_amount, status_id, client_id, clients (name), order_id, orders (order_number), created_at, deleted_at')
      .eq('id', id)
      .maybeSingle()
...
export async function createInvoice(invoiceData) {
  try {
    ...
    if (invoice_items && invoice_items.length > 0) {
      ...
      if (itemsError)
        console.warn('Invoice items insert warning:', itemsError.message)
    }
...
export async function updateInvoice(invoiceId, invoiceData) {
  try {
    ...
    const currentStatusCode = existing.status?.code
