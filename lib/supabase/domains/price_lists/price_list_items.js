...
export async function getPriceListItems(filters) {
  try {
    ...
    let query = supabase
      ...
      .select('id, price_list_id, price_lists (name), product_id, products (name), price, deleted_at', { count: 'exact' })
...
export async function getPriceListItemById(itemid) {
  try {
    ...
    const { data, error } = await supabase
      .from('price_list_items')
      .select('id, price_list_id, price_lists (name), product_id, products (name), price, deleted_at')
      .eq('id', itemid)
      .maybeSingle()
