...
export async function getClientContacts(clientId, filters) {
  try {
    ...
    let query = supabase
      ...
      .select('id, client_id, clients (name), contact_type_id, contact_types (code, name), value, deleted_at', { count: 'exact' })
    ...
    if (search) {
      ...
      if (matchedClientIds.length > 0) {
        query = query.or(`value.ilike.%${search}%,client_id.in.(${matchedClientIds.join(',')})`);
      } else {
    ...
    if (contact_type_id) {
      query = query.eq('contact_type_id', contact_type_id)
    }
...
export async function updateClientContact(contactId, clientId, contactData) {
  try {
    if (!contactId) throw new Error('Contact ID is required')
    await verifyAccessLevel(2)//manager+

    const { contact_type_id, value, client_id } = contactData
    ...
    const { data: existing } = await supabase
      .from('client_contacts')
      .select('id, contact_type_id, client_id')
      .eq('id', contactId)
      .single()
    ...
    return await getClientContactById(contactId, clientId)
  } catch (error) {
