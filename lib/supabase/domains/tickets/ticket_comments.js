...
export async function getTicketComments(ticketId, filters) {
  try {
    ...
    let query = supabase
      ...
      .select('id, ticket_id, tickets (ticket_number), user_id, user_profiles (first_name, last_name), message, created_at', { count: 'exact' })
    ...
    const extendedData = data?.map(comment => ({
      ...comment,
...
export async function getTicketCommentById(commentId) {
  try {
    ...
    const { data, error } = await supabase
      .from('ticket_comments')
      .select('id, ticket_id, tickets (ticket_number), user_id, user_profiles (first_name, last_name), message, created_at, tickets!inner (created_by, ticket_number)')
      .eq('id', commentId)
      .maybeSingle()
    ...
    return {
      ...data,
      user_name: data.user_profiles 
        ? `${data.user_profiles.first_name} ${data.user_profiles.last_name}`.trim()
        : 'System User'
    }
  } catch (error) {
...
export async function updateTicketComment(commentId, commentData) {
  try {
    if (!commentId) throw new Error('Comment ID is required')
    ...
    return await getTicketCommentById(commentId)
  } catch (error) {
