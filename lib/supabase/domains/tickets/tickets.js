...
export async function getTicketById(id) {
  try {
    ...
    const { data: ticket, error: ticketError } = await supabase
      .from('tickets')
      .select('id, ticket_number, client_id, subject, description, priority_id, ticket_priorities(name, level), status_id, ticket_statuses (name), clients (name), assigned_to, created_by, created_at, deleted_at, ticket_comments(id, message, created_at, user_id,        user_profiles(first_name, last_name))')
      .eq('id', id)
      .maybeSingle()
