...
export async function getTickets(filters) {
  try {
    ...
    let query = supabase
      .from('tickets')
      .select('id, ticket_number, client_id, subject, description, priority_id, ticket_priorities(name, level), status_id, ticket_statuses (name), clients (name), assigned_to, users!tickets_assigned_to_fkey (email), created_by, created_at, deleted_at', { count: 'exact' })

    //filter active only
		if (activeOnly)
		  query = query.is('deleted_at', null)
    
    // subject, description searching
    if (search) {
      query = query.or(
...
export async function getTicketById(id) {
  try {
    ...
    const { data: ticket, error: ticketError } = await supabase
      .from('tickets')
      .select('id, ticket_number, client_id, subject, description, priority_id, ticket_priorities(name, level), status_id, ticket_statuses (name), ticket_attachments (*), clients (name), assigned_to, users!tickets_assigned_to_fkey (email), created_by, created_at, deleted_at, ticket_comments(id, message, created_at, user_id, user_profiles(first_name, last_name))')
      .eq('id', id)
      .maybeSingle()
    ...
    //create user names from user_profiles
    ...
    if (userIds.length > 0) {
      ...
      profiles?.forEach(profile => {
        ...
        if (profile.user_id === ticket.created_by) created_by_name = fullName
        if (profile.user_id === ticket.assigned_to) assigned_to_name = fullName
      })
    }
