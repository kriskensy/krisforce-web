...
import { getPaymentMethods } from "../supabase/domains/enumerations/payment_methods";
...
export async function recordPaymentAction(invoiceId, amount, paymentMethodId) {
  try {
    const result = await recordPayment(invoiceId, amount, paymentMethodId);
