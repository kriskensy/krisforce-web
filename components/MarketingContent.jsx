...
export function MarketingContent({ content }) {

  //for easier access
  const getCmsMarketignValue = (key, fallback) => content?.find(item => item.key === key)?.value || fallback;
    ...
    <div className="relative overflow-hidden bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[2.5rem] p-8 md:p-12 shadow-2xl ring-1 ring-black/5 w-full max-w-5xl mx-auto">
      ...
      <div className="text-center mb-12">
        <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-white mb-4">
          {getCmsMarketignValue('marketing_headline', 'KrisForce 2.0 Ecosystem')}
        </h2>
        <p className="text-lg text-white/60 max-w-2xl mx-auto">
          {getCmsMarketignValue('marketing_subheadline', 'Everything you need to manage your business.') }
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12 text-left">
        <FeatureItem 
          ...
          title={getCmsMarketignValue('marketing_f1_title', 'Advanced CRM')}
          description={getCmsMarketignValue('marketing_f1_desc', 'Manage leads and relationships.')}
        />
        <FeatureItem 
          icon={<BarChart3 className="text-emerald-400" />}
          title={getCmsMarketignValue('marketing_f2_title', 'Real-time Analytics')}
          description={getCmsMarketignValue('marketing_f2_desc', 'Track performance with charts.')}
        />

        <FeatureItem 
          icon={<ShieldCheck className="text-purple-400" />}
          title={getCmsMarketignValue('marketing_f3_title', 'Enterprise Security')}
          description={getCmsMarketignValue('marketing_f3_desc', 'Bank-grade encryption.')}
        />
